...
/**
 * Properties: ignore.malformed.json = true/false : malformed json will be
...
public class JsonSerDe implements SerDe {
	...
	/**
	 * Initializes the SerDe. Gets the list of columns and their types from the
	 * table properties. Will use them to look into/create JSON data.
	 * 
	 * @param conf
	...
	public void initialize(Configuration conf, Properties tbl)
		...
		LOG.debug("columns " + columnNameProperty + " types "
				+ columnTypeProperty);
		...
		// other configuration
		ignoreMalformedJson = Boolean.parseBoolean(tbl.getProperty(
				PROP_IGNORE_MALFORMED_JSON, "false"));

	}

	/**
	 * Deserializes the object. Reads a Writable and uses JSONObject to parse
	 * its text
	 * 
	 * @param w
	 *            the text to parse
	...
	/**
	 * Hive will call this to serialize an object. Returns a writable object of
	 * the same class returned by <a
	 * href="#getSerializedClass">getSerializedClass</a>
	 * 
	 * @param obj
	 *            The object to serialize
	 * @param objInspector
	 *            The ObjectInspector that knows about the object's structure
	 * @return a serialized object in form of a Writable. Must be the same type
	...
	public Writable serialize(Object obj, ObjectInspector objInspector)
		...
		JSONObject serializer = serializeStruct(obj,
				(StructObjectInspector) objInspector, columnNames);
	...
	private String getSerializedFieldName(List<String> columnNames, int pos,
			StructField sf) {
		String n = (columnNames == null ? sf.getFieldName() : columnNames
				.get(pos));
	 ...
	 * JSONObject field. Actual serialization is done at the end when the whole
	 * JSON object is built
	 */
	private JSONObject serializeStruct(Object obj, StructObjectInspector soi,
			List<String> columnNames) {
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			if (null != data) {
				try {
					...
					result.put(getSerializedFieldName(columnNames, i, sf),
	...
	Object serializeField(Object obj, ObjectInspector oi) {
		...
		case PRIMITIVE:
			...
			case VOID:
				...
				break;
			case BOOLEAN:
				result = (((BooleanObjectInspector) poi).get(obj) ? Boolean.TRUE
						: Boolean.FALSE);
				break;
			case BYTE:
			...
			case SHORT:
				...
				break;
			case STRING:
				result = (((StringObjectInspector) poi)
	...
	private JSONObject serializeMap(Object obj, MapObjectInspector moi) {
		...
		for (Object k : m.keySet()) {
			try {
				jo.put(serializeField(k, moi.getMapKeyObjectInspector())
						.toString(),
						serializeField(m.get(k),
